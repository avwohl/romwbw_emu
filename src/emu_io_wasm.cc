...
//=============================================================================
// Disk Image Creation Implementation
//=============================================================================

// Format a single hd1k slice (8MB) with empty CP/M directory
// offset: byte offset in data where slice starts
static void format_hd1k_slice(std::vector<uint8_t>& data, size_t offset) {
  // hd1k format:
  // - 512 bytes/sector, 16 sectors/track, 1024 tracks
  // - Block size: 4096 bytes (8 sectors)
  // - Boot tracks: 2 (reserved)
  // - Directory: 1024 entries × 32 bytes = 32KB = 8 blocks
  //
  // Directory starts at track 2, sector 0
  // Directory offset = 2 tracks × 16 sectors × 512 bytes = 16,384 bytes
  // Directory size = 32,768 bytes (1024 entries × 32 bytes)

  const size_t BOOT_TRACKS = 2;
  const size_t SECTORS_PER_TRACK = 16;
  const size_t BYTES_PER_SECTOR = 512;
  const size_t DIR_ENTRIES = 1024;
  const size_t DIR_ENTRY_SIZE = 32;

  size_t dir_offset = offset + (BOOT_TRACKS * SECTORS_PER_TRACK * BYTES_PER_SECTOR);
  size_t dir_size = DIR_ENTRIES * DIR_ENTRY_SIZE;

  // Initialize directory with 0xE5 (CP/M empty directory marker)
  if (dir_offset + dir_size <= data.size()) {
    memset(&data[dir_offset], 0xE5, dir_size);
  }
}

// Create MBR for combo disk with RomWBW partition type
static void create_combo_mbr(std::vector<uint8_t>& data) {
  // MBR structure:
  // - Offset 0x1BE: First partition entry (16 bytes)
  // - Offset 0x1FE: Signature 0x55AA
  //
  // Partition entry structure:
  // - Offset 0: Boot flag (0x00 = not bootable, 0x80 = bootable)
  // - Offset 1-3: CHS start
  // - Offset 4: Partition type (0x2E = RomWBW hd1k)
  // - Offset 5-7: CHS end
  // - Offset 8-11: LBA start (little-endian)
  // - Offset 12-15: LBA size (little-endian)

  // First partition starts at 1MB (LBA 2048 with 512-byte sectors)
  const uint32_t PARTITION_START_LBA = 2048;  // 1MB / 512 = 2048 sectors
  const uint32_t PARTITION_SIZE_LBA = (EMU_HD1K_COMBO_SIZE - EMU_HD1K_MBR_PREFIX) / 512;

  // Partition entry at offset 0x1BE
  size_t part_offset = 0x1BE;
  ...
  data[part_offset + 8] = (PARTITION_START_LBA >> 0) & 0xFF;
  data[part_offset + 9] = (PARTITION_START_LBA >> 8) & 0xFF;
  data[part_offset + 10] = (PARTITION_START_LBA >> 16) & 0xFF;
  data[part_offset + 11] = (PARTITION_START_LBA >> 24) & 0xFF;

  // LBA size (little-endian)
  data[part_offset + 12] = (PARTITION_SIZE_LBA >> 0) & 0xFF;
  data[part_offset + 13] = (PARTITION_SIZE_LBA >> 8) & 0xFF;
  data[part_offset + 14] = (PARTITION_SIZE_LBA >> 16) & 0xFF;
  data[part_offset + 15] = (PARTITION_SIZE_LBA >> 24) & 0xFF;
...
std::vector<uint8_t> emu_disk_create_memory(emu_disk_format format) {
  ...
  if (format == EMU_DISK_HD1K_SINGLE) {
    // Single slice: format the entire disk
    format_hd1k_slice(data, 0);
  } else if (format == EMU_DISK_HD1K_COMBO) {
    // Combo disk: create MBR and format each slice
    create_combo_mbr(data);

    // Format each slice (starts after 1MB MBR prefix)
    for (size_t slice = 0; slice < EMU_HD1K_COMBO_SLICES; slice++) {
      size_t slice_offset = EMU_HD1K_MBR_PREFIX + (slice * EMU_HD1K_SLICE_SIZE);
...
bool emu_disk_create(const std::string& path, emu_disk_format format) {
  // In WASM, we use Emscripten's virtual filesystem
  // Create disk image in memory first
  std::vector<uint8_t> data = emu_disk_create_memory(format);
  if (data.empty()) {
    return false;
  }
