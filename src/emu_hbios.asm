...
; is generated by whichever builder is assembling this file - romwbw_disks'
; tools/build_rom.sh, from versions/<ver>/version.json, or romwbw_emu's
; roms/build_emu_rom.sh, from the HCB of the stock ROM it is overlaying - so
; one source builds a ROM for any RomWBW release and the two stamps below can
; never drift from each other, from the stock banks, or from the published
; catalog.  See romwbw_disks/docs/INTERFACE_V0.md.
;
; THIS FILE IS BYTE-IDENTICAL IN romwbw_disks AND romwbw_emu, and
; romwbw_disks/tools/check_source_drift.sh asserts that.  Edit both or
; neither.  An edit moves every published ROM's sha256, which bumps each
; release's catalog generation, which makes every client re-download its
; disk images - budget that before changing anything here.
;
...
;
; Assembling this needs romwbw_ver.inc beside it, which a builder generates -
; romwbw_disks' tools/build_rom.sh or romwbw_emu's roms/build_emu_rom.sh.  Run
; one of those rather than um80 by hand; by hand it is:
;   printf 'RMV_VER\tequ\t036h\nRMV_UPD\tequ\t000h\n' > romwbw_ver.inc
;   um80 -g emu_hbios.asm && ul80 -o emu_hbios.bin -p 0000 emu_hbios.rel
;
;==================================================================================================
